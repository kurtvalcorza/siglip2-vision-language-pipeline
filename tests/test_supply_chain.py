...
def test_resolve_weights_path_precedence(
    ...
    monkeypatch.setenv("SIGLIP2_WEIGHTS_DIR", str(env_dir))
    resolved, source = model_module.resolve_weights_path()
    assert resolved == env_dir
    assert source == "env_var"

    monkeypatch.delenv("SIGLIP2_WEIGHTS_DIR", raising=False)
    ...
    fake_pkg_file = tmp_path / "site-packages" / "siglip2_pipeline" / "model.py"
