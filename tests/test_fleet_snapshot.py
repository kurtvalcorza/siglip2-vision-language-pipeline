...
from siglip2_pipeline import model as model_mod
...
def test_verify_snapshot_refuses_a_missing_manifest_entry(pinned_to_stand_in):
    (pinned_to_stand_in / "tokenizer.model").unlink()
    with pytest.raises(RuntimeError, match="Manifest file missing: tokenizer.model"):
        verify_snapshot(pinned_to_stand_in)
